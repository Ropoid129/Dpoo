...
public class SandboxListas
{
    ...
    {
    	int[] copyArray = new int[listaEnteros.size()];

        for (int index = 0; index < listaEnteros.size(); index++) {
            copyArray[index] = listaEnteros.get(index);
    ...
    {
    	listaCadenas.removeIf((stringArray) -> {
            return stringArray.equals(cadena);
        });
    ...
    {
    	listaEnteros.replaceAll((number) -> {
            if(number < 0) {
                return number * -1;
            }
            return number;
    ...
    public int contarEnterosRepetidos( )
    {
        ...
        for (Integer integer : listaEnteros) {
            if (!uniqueElements.add(integer)) {
                repeatedElements.add(integer);
            }
        }

        count = repeatedElements.size();
        return count;
    }
